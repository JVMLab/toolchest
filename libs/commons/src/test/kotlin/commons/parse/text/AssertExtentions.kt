...
fun ComplexToken<TokenType>.assertComplexEquals(
    expectedToken: ComplexToken<TokenType>,
  ...
  subTokens.forEachIndexed { idx, t ->
    ...
    val message = "sub-token $expectedSubTokenType #$idx"
    if ((t is ComplexToken) && (expectedSubToken is ComplexToken)) {
      t.assertComplexEquals(expectedSubToken, message)
    } else {
      t.assertEquals(expectedSubToken, message)
    }
  }
}
...
fun Parsed<CharSequence, List<Token<TokenType>>>.assertEquals(
  ...
  result[ParsedKey.PARSED_STRING.key]?.forEachIndexed { idx, t ->
    ...
    val message = "token $expectedTokenType #$idx"
    if ((t is ComplexToken) && (expectedToken is ComplexToken)) {
      t.assertComplexEquals(expectedToken, message)
    } else {
      t.assertEquals(expectedToken, message)
    }
  }
